<?php
include 'header.php';
$connfig_name = "product";
$connfig_title = "sản phẩm";
if (!empty($_SESSION['current_user'])) {
    if(!empty($_GET['action']) && $_GET['action'] == 'search' && !empty($_POST)){
        $_SESSION[$connfig_name.'_filter'] = $_POST;
        header('Location: '.$connfig_name.'_listing.php');exit;
    }
    if(!empty($_SESSION[$connfig_name.'filter'])){
        $where = "";
        foreach ($_SESSION[$connfig_name.'filter'] as $field => $value) {
            if(!empty($value)){
                switch ($field) {
                    case 'name':
        ...
        extract($_SESSION[$connfig_name.'filter']);
    }
    ...
    <div class="main-content">
        <h1>Danh sách <?= $connfig_title ?></h1>
        <div class="listing-items">
            <div class="buttons">
                <a href="./<?= $connfig_name ?>_editing.php">Thêm <?= $connfig_title ?></a>
            </div>
            <div class="listing-search">
                <form id="<?= $connfig_name ?>-search-form" action="<?= $connfig_name ?>_listing.php?action=search" method="POST">
                    <fieldset>
                        <legend>Tìm kiếm <?= $connfig_title ?>:</legend>
                        ID: <input type="text" name="id" value="<?= !empty($id) ? $id : "" ?>" />
                        Tên <?= $connfig_title ?>: <input type="text" name="name" value="<?= !empty($name) ? $name : "" ?>" />
            ...
            <div class="total-items">
                <?php /*
                  <span>Có tất cả <strong><?=$totalRecords?></strong> <?=$connfig_title?> trên <strong><?=$totalPages?></strong> trang</span> */ ?>
            </div>
            <ul>
                ...
                <li>
                    ...
                    <div class="listing-prop listing-button">
                        <a href="../hoadon.php?id=<?=$row['id']?>" target="_blank">In</a>
                    </div>
